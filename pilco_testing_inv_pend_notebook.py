...
from jax_mc_pilco.model_learning.dynamical_models import IMGPR, IMSGPR, optimize_imgpr, optimize_imsgpr
...
for trial in range(2):
    ...
    if len(states) > 500:
        ...
        if 'inducing_point_locations' not in params[0].keys():
           initial_inducing_points = jnp.linspace(jnp.array(states).min(), jnp.array(states).max(), num=num_inducing_points, endpoint=True)
           for i in range(state_dim):
               params[i]['inducing_point_locations'] = np.array(initial_inducing_points,dtype=jnp.float64)
        model = IMSGPR(
            states=states_array,
        ...
        model = optimize_imsgpr(model,states=states_array,actions=actions_array,)
        ...
        start_time = time.perf_counter()
        model = optimize_imgpr(model,states=states_array,actions=actions_array,)
        end_time = time.perf_counter()
        print(f'Model Optimization Time = {end_time-start_time}')
    ...
    cosine_decay_scheduler = optax.cosine_decay_schedule(0.0001, decay_steps=num_opt_steps, alpha=0.95)
