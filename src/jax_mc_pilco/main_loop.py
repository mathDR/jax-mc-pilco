...
class ReplayBuffer(eqx.Module):
    ...
    def add(self, transition: TransitionBatch) -> "ReplayBuffer":
        ...
        new_buffers: TransitionBatch = jax.tree_util.tree_map(
            lambda buf, new_data: buf.at[indices].set(new_data), self.buffers, transition
        )
...
class FlowPolicy(eqx.Module):
    ...
    def __init__(self, action_dim: int, obs_dim: int, key: jtp.Key[jtp.Array, ""]) -> None:
        self.flow = masked_autoregressive_flow(
            key=key, base_dist=Normal(jnp.zeros(action_dim)), cond_dim=obs_dim, nn_width=64, nn_depth=2, flow_layers=3
        )
...
@eqx.filter_value_and_grad
def policy_loss_fn(
    policy: FlowPolicy, critic: CriticNetwork, obs: jax.Array, alpha: float, key: jtp.Key[jtp.Array, ""]
) -> jax.Array:
    keys: jax.Array = jax.random.split(key, obs.shape[0])
...
class MJXRolloutManager:
    ...
    def collect_step(
        self, policy: FlowPolicy, env_state: typing.Any, key: jtp.Key[jtp.Array, ""]
    ) -> tuple[typing.Any, TransitionBatch]:
...
def training_iteration(
    ...
    def do_update(
        s: TrainState,
    ) -> tuple[FlowPolicy, CriticNetwork, CriticNetwork, optax.OptState, optax.OptState, jax.Array]:
        batch: TransitionBatch = s.buffer.sample(sample_key, config["batch_size"])

        # Optimize Critic
        c_loss, c_grads = critic_loss_fn(
            s.critic,
            s.policy,
            s.target_critic,
            batch.obs,
    ...
    next_state = TrainState(
        ...
        opt_state_p=opt_state_p,
        opt_state_c=opt_state_c,
        buffer=new_buffer,
        env_state=next_env_state,
        current_obs=next_env_state.obs,
    )
