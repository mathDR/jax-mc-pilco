...
# Must be set before `import mujoco`: MuJoCo selects its OpenGL backend at
# import time. "osmesa" is the software (CPU) renderer that works headlessly,
# without a display or GPU - exactly what's needed to render an mp4 on a
# server/container. If you're running with a display and GPU available, you
# can delete this line (or set MUJOCO_GL=glfw/egl before running the script).
...
# ----------------------------------------------------------------------------
def make_initial_carry(key: jax.Array):
...
@eqx.filter_jit
def rollout(agent: Agent, data0: mjx.Data, prev0, key, n_steps: int):
    def step(carry, key):
...
@eqx.filter_jit
def batched_rollout(agent: Agent, keys: jax.Array, n_steps: int):
...
# ----------------------------------------------------------------------------
def compute_gae(rewards, values, dones, last_value, gamma: float = 0.99, lam: float = 0.95):
...
def ppo_loss(agent: Agent, batch, clip_eps: float, vf_coef: float, ent_coef: float):
    ...
    return loss, dict(policy_loss=policy_loss, value_loss=value_loss, entropy=entropy_bonus)
...
def ppo_minibatch_step(agent, opt_state, optimizer, mb, clip_eps, vf_coef, ent_coef):
    ...
    info["skipped"] = jnp.logical_not(grad_is_finite).astype(jnp.float32)
...
def train(
    ...
    for it in range(n_iterations):
        ...
        skip_rate = jnp.mean(infos["skipped"])
        print(
        ...
        if skip_rate > 0.5:
            print(
                "  ^ WARNING: over half of minibatches this iteration had a "
                "non-finite gradient and were skipped - the actor is "
                "effectively frozen, not actually training. See the note in "
                "ppo_minibatch_step."
            )
...
# ----------------------------------------------------------------------------
# 8. Watch the trained policy: roll out one episode, render it, save an mp4
# ----------------------------------------------------------------------------
def eval_rollout_qpos(agent: Agent, key: jax.Array, n_steps: int, temperature: float = 1.0):
    """Single-env rollout returning the qpos trajectory needed for rendering,
    plus reward/done for reporting. `temperature` < 1 makes the (already
    trained) flow's action sampling more deterministic/repeatable - see the
    note on FlowActor.sample_action."""

    def step(carry, key):
        data, prev_state = carry
        curr_state = obs_from_data(data)
        action = agent.actor.sample_action(key, prev_state, curr_state, temperature=temperature)
        next_data = mjx.step(mjx_model, data.replace(ctrl=action))
        reward, done = reward_and_done(next_data)
...
def render_video(
    ...
    key = jax.random.key(seed)
    qpos_traj, rewards, dones = eval_rollout_qpos(agent, key, n_steps, temperature=temperature)
    ...
    # Fall back to the default free camera if the named camera isn't defined
    # on this model (e.g. an XML that hasn't been updated with the `side`
    # camera yet), rather than hard-failing mid-render.
    camera_names = [mj_model.camera(i).name for i in range(mj_model.ncam)]
    if camera not in camera_names:
        print(
            f"Warning: camera '{camera}' not found in model "
            f"(available: {camera_names or 'none'}); using the default free camera. "
            f'Add <camera name="{camera}" pos="0 -4 1.2" xyaxes="1 0 0 0 0 1"/> '
            "inside <worldbody> to get the intended side view."
        )
        camera = -1  # mujoco.Renderer's free-camera sentinel
    ...
    # Write frames to disk as PNGs and invoke ffmpeg on the image sequence,
    # rather than piping raw frames into ffmpeg's stdin (imageio's default
    # writer does this via a forked subprocess with an open pipe, which is
    # the specific combination flagged by Python's "os.fork() is incompatible
    # with multithreaded code" warning - risky here since JAX keeps its own
    # thread pool alive). A plain subprocess.run() over files uses posix_spawn
    # instead of fork on POSIX, sidestepping the deadlock risk entirely.
    with tempfile.TemporaryDirectory() as tmpdir:
        for i, frame in enumerate(frames):
            imageio.imwrite(f"{tmpdir}/frame_{i:05d}.png", frame)
...
if __name__ == "__main__":
    # Train on the same episode length you'll evaluate/watch afterward -
    # otherwise the policy is only ever optimized for the shorter training
    # horizon and has no reason to have learned to hold balance beyond it.
    trained_agent = train(n_envs=32, n_steps=300, n_iterations=100, ppo_epochs=4, minibatch_size=256)
    render_video(trained_agent, "pendulum_policy.mp4", n_steps=300, seed=123, temperature=1.0)
